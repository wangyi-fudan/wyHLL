...
static	inline	uint32_t	wyset_alloc(uint8_t	**data,	uint64_t	bits){	*data=(uint8_t*)calloc(1ull<<bits,1);	return	data!=NULL;	}

//	reset the registers
static	inline	void	wyset_clear(uint8_t	*data,	uint64_t	bits){	memset(data,0,1ull<<bits);	}

//	merge two sets
static	inline	void	wyset_merge(uint8_t	*des,	uint8_t	*src,	uint64_t	bits){
...
//	add an element
static	inline	void	wyset_add(uint8_t	*data,	uint64_t	bits,	void	*item,	uint64_t	item_size){
...
static	inline	double	wyset_bf_estimator(uint8_t	*data,	uint64_t	bits){
...
//	HLL estimaator
static	inline	double	wyset_hll_estimator(uint8_t	*data,	uint64_t	bits){
